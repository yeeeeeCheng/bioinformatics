...
\vspace{1 cm}
{\centering\large Rsubread v2.22.1/Subread v2.1.1\\}
\vspace{1 cm}
\centering 16 April 2025\\
\vspace{5 cm}
...
Melbourne, Australia\\}
\vspace{7 cm}
\centering Copyright \small{\copyright}  2011 - 2025\\
\end{center}
...
\begin{quote}
Liao Y, Raghu D, Pal B, Mielke LA and Shi W (2023).
cellCounts: an R function for quantifying 10x Chromium single-cell RNA sequencing data. 
\emph{Bioinformatics}, 39(7):btad439.
\\
{\color{blue}{\url{http://www.ncbi.nlm.nih.gov/pubmed/37462540}} }
\end{quote}
...
The output of {\featureCounts} program includes a count table that contains read counts for features such as genes. 
In SourceForge {\Subread}, the count table is saved in a tab-delimited file (file name is specified by the user via the \code{-o} option).
In {\Rsubread}, the count table is saved in the `counts' component of the \code{list} object returned by {\featureCounts} (for more details see the help page for {\featureCounts} function in {\Rsubread} package). 
...
\subsubsection{Counting summary}

{\featureCounts} also generates a summary of counting results.
In SourceForge {\Subread}, the counting summary is saved in a tab-delimited file, named by appending `.summary' to the read count file.
In {\Rsubread}, the counting summary is stored in the `stat' component of the \code{list} object returned by {\featureCounts} (for more details see the help page for {\featureCounts} function in {\Rsubread} package). 
...
\begin{itemize}
\item \code{Unassigned\_Unmapped}: unmapped reads cannot be assigned.
\item \code{Unassigned\_Read\_Type}: reads that have an unexpected read type (eg. being a single end read included in a paired end dataset) and also cannot be counted with confidence (eg. due to stranded counting). Such reads are typically generated from a read trimming program.
\item \code{Unassigned\_Singleton}: read pairs that have only one end mapped. 
\item \code{Unassigned\_MappingQuality}: alignments with a mapping quality score lower than the threshold.
\item \code{Unassigned\_Chimera}: two ends in a paired end alignment are located on different chromosomes or have unexpected orientation.
\item \code{Unassigned\_FragementLength}: fragment length inferred from paired end alignment does not meet the length criteria.
\item \code{Unassigned\_Duplicate}: alignments marked as duplicate (indicated in the FLAG field).
\item \code{Unassigned\_MultiMapping}: alignments reported for multi-mapping reads (indicated by `NH' tag).
\item \code{Unassigned\_Secondary}: alignments reported as secondary alignments (indicated in the FLAG field). 	
\item \code{Unassigned\_Split} (or \code{Unassigned\_NonSplit}): alignments that contain junctions (or do not contain junctions).
\item \code{Unassigned\_NoFeatures}: alignments that do not overlap any feature.
\item \code{Unassigned\_Overlapping\_Length}: alignments that do not overlap any feature (or meta-feature) with the minimum required overlap length.
\item \code{Unassigned\_Ambiguity}: alignments that overlap two or more features (feature-level summarization) or meta-features (meta-feature-level summarization).
\end{itemize}
...
\subsubsection{Junction counts}
\label{sec:junction_counts}

{\featureCounts} counts the number of reads supporting each exon-exon junction and provides additional information about junctions and their splicing points when \code{-J} is specified (or \code{juncCounts=TRUE} in {\R}).
In SourceForge {\Subread}, the result is saved in a tab-delimited file, named by appending `.jcounts' to the read count output file specified via the \code{-o} option.
In {\Rsubread}, the result is stored in the \code{counts\_junction} component of the \code{list} object returned by {\featureCounts} (for more details see the help page for {\featureCounts} function in {\Rsubread} package). 
The result includes the following columns:

\begin{itemize}
\item \code{Gene\_SP1}: Gene(s) annotated for a splicing point. This represents one of the two splicing points associated with an exon-exon junction. A splicing point refers to the chromosomal base immediately before the donor site or after the acceptor site. The annotation provided to featureCounts is used to label junctions and their corresponding splicing points. Gene(s) associated with a splicing point are determined based on the following priority order: (1) genes containing a transcript that supports the existence of this junction; (2) genes containing an exon whose start or end base coincides with this splicing point; (3) genes within whose span this splicing point is located. If no such genes are found, the value will be ``NA''. If an \code{SAF} format annotation is provided, the first priority will be ignored as the \code{SAF} annotation does not contain transcript information.
\item \code{Gene\_SP2}: See description for \code{Gene\_SP1}.
\item \code{Transcript}: Transcript(s) that support this junction. If none are found, the value is ``NA''.
\item \code{Status}: If at least one transcript in the provided annotation supports the junction, the value is ``KNOWN''; otherwise, it is ``NOVEL''. ``NA'' if an \code{SAF} annotation is provided.
\item \code{Donor}: If the junction's donor and acceptor sites can be identified based on the canonical donor/acceptor sequences (GT/AG), the value will be ``SP1'' or ``SP2''. Otherwise, it will be ``NA''.
\item \code{Acceptor}: See description for \code{Donor}.
\item \code{Chr\_SP1}: Name of chromosome where splicing point 1 is located.
\item \code{Location\_SP1}: Chromosomal coordinate of splicing point 1.
\item \code{Strand\_SP1}: Strand of splicing point 1, which can be ``+'', ``-'', or ``NA'' if unknown. Strandness is determined based on the donor/acceptor sequences (GT/AG).
\item \code{Chr\_SP2}: see description for \code{Chr\_SP1}.
\item \code{Location\_SP2}: see description for \code{Location\_SP1}.
\item \code{Strand\_SP2}: see description for \code{Strand\_SP1}.
\item \code{NearestExonBoundary\_SP1}: Providing information about the nearest exon boundary (either the start or end base of an exon) relative to the splicing point, including chromosomal coordinate, gene name, distance, and direction (left or right of the splicing point). If the splicing point coincides with an exon boundary (as expected in most cases), the distance will be 0, and direction information will be omitted. Example formats: ``chr1:1000,genename,0'', or ``chr2:2000,genename,100,left''.
\item \code{NearestExonBoundary\_SP2}: See description for \code{NearestExonBoundary\_SP1}.
...
\hline
-J \newline (\code{juncCounts}) & Count the number of reads supporting each exon-exon junction. Junctions will be identified from all the exon-spanning reads (containing `N' in CIGAR string) included in the input data (note that options `--splitOnly' and `--nonSplitOnly' are not considered by this parameter). In addition to junction counts, the output includes additional information on detected exon-exon junctions and their splicing points. A splicing point refers to the chromosomal base immediately before the donor site or after the acceptor site. In the {\C} version of {\featureCounts}, the output is saved to a file named `$<$output\_file$>$.jcounts', where `$<$output\_file$>$' is specified in the \code{-o} option. In the {\Rsubread} {\featureCounts} function, the result is stored in a data frame named \code{counts\_junction}, which is included in the \code{list} object returned by {\featureCounts}. The output includes the following columns: \code{Gene\_SP1}, \code{Gene\_SP2}, \code{Transcript}, \code{Status}, \code{Donor}, \code{Acceptor}, \code{Chr\_SP1}, \code{Location\_SP1}, \code{Strand\_SP1}, \code{Chr\_SP2}, \code{Location\_SP2}, \code{Strand\_SP2}, \code{NearestExonBoundary\_SP1}, \code{NearestExonBoundary\_SP2}, and columns for junction counts. See Section \ref{sec:junction_counts} for a description of these columns.\\
\hline
